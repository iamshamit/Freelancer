...
const createMilestones = async (req, res) => {
  try {
    ...
    if (job.status !== "assigned") {
      return res
        .status(400)
        .json({ message: "Job must be assigned before creating milestones" });
    }
    ...
    if (existingMilestones.length > 0) {
      return res
        .status(400)
        .json({ message: "Milestones already exist for this job" });
    }
    ...
    if (totalPercentage !== 100) {
      return res
        .status(400)
        .json({ message: "Total milestone percentage must equal 100%" });
    }
...
const getMilestones = async (req, res) => {
  try {
    ...
    const milestones = await Milestone.find({ job: req.params.jobId }).sort({
      order: 1,
    });
...
const requestMilestoneApproval = async (req, res) => {
  try {
    ...
    if (milestone.status !== "in_progress") {
      return res
        .status(400)
        .json({ message: "Milestone must be in progress to request approval" });
    }
...
const approveMilestone = async (req, res) => {
  try {
    ...
    if (milestone.status !== "completed") {
      return res
        .status(400)
        .json({ message: "Milestone must be completed before approval" });
    }
...
const rejectMilestone = async (req, res) => {
  try {
    ...
    if (milestone.status !== "completed") {
      return res
        .status(400)
        .json({ message: "Can only reject completed milestones" });
    }
...
const updateMilestone = async (req, res) => {
  try {
    ...
    if (milestone.status === "approved") {
      return res
        .status(400)
        .json({ message: "Cannot update approved milestones" });
    }
...
const deleteMilestone = async (req, res) => {
  try {
    ...
    if (milestone.status !== "pending") {
      return res
        .status(400)
        .json({ message: "Can only delete pending milestones" });
    }
    ...
    const remainingPercentage = otherMilestones.reduce(
      (sum, m) => sum + m.percentage,
      0,
    );
    ...
    const remainingMilestones = await Milestone.find({ job: jobId }).sort({
      order: 1,
    });
