  ...
  try {
    ...
    const payments = await Promise.all(
      transactions.map(async (transaction) => {
        const job = await Job.findById(transaction.job).populate(
          "employer freelancer",
        );
...
const getPaymentReceipt = async (req, res) => {
  try {
    ...
    doc.text("Receipt Details", leftColumn, doc.y, {
      underline: true,
      width: 200,
    });
    doc.text("Transaction Info", rightColumn, doc.y - 15, {
      underline: true,
      width: 200,
    });
    ...
    doc
      .text(`Receipt #: ${receiptId}`, leftColumn)
    ...
    if (transaction.percentage) {
      ...
      doc.text(
        `Milestone: ${milestone ? milestone.title : `${transaction.percentage}% Complete`}`,
      );
    }
    ...
    if (isUserEmployer) {
      doc
        .text(`Client (You): ${transaction.from.name}`)
      ...
      doc
        .text(`Freelancer (You): ${transaction.to.name}`)
        .text(`Email: ${transaction.to.email}`)
        .moveDown(0.5)
      ...
      doc
        .text(`From: ${transaction.from.name} (${transaction.from.email})`)
    ...
    // Payment summary box
    const boxY = doc.y;
    doc.rect(leftColumn, boxY, 500, 60).strokeColor("#e5e7eb").stroke();
    ...
    doc
      ...
      .fillColor("#6b7280")
      .text(
        "This is an official receipt for the transaction completed on FreelanceHub platform.",
        { align: "center", y: footerY },
      )
      .text(
        "For support or questions, please contact support@freelancehub.com",
        { align: "center" },
      )
      .text(
        ...
        transactions.map(async (t) => {
          ...
          if (t.percentage) {
            ...
            milestoneInfo = milestone
              ? milestone.title
              : `${t.percentage}% Milestone`;
          }

          // Get job details
          const job = await Job.findById(t.job._id).populate(
            "employer freelancer",
          );
