...
const getDashboardStats = async (req, res) => {
  try {
    ...
    const [newUsersLast30Days, newJobsLast30Days, transactionsLast30Days] =
      await Promise.all([
...
const getUserDetails = async (req, res) => {
  try {
    ...
    const [userJobs, userTransactions, userChats] = await Promise.all([
      Job.find({
        $or: [{ employer: user._id }, { selectedFreelancer: user._id }],
      }).lean(),
      Transaction.find({
        $or: [{ from: user._id }, { to: user._id }],
      }).lean(),
      Chat.find({
        $or: [{ employer: user._id }, { freelancer: user._id }],
      }).lean(),
    ]);
    ...
    res.json({
      ...
      statistics: {
        ...
        completedJobs: userJobs.filter((job) => job.status === "completed")
          .length,
  ...
  try {
    ...
    if (job.status === "active" && job.selectedFreelancer) {
      return res.status(400).json({
        message:
          "Cannot delete job with active freelancer. Please resolve first.",
      });
    }
  ...
  try {
    ...
    if (adminId && adminId !== "all") {
      allActions = allActions.filter(
        (a) => a.performedBy._id.toString() === adminId,
      );
    }

    if (dateFrom) {
      allActions = allActions.filter(
        (a) => new Date(a.createdAt) >= new Date(dateFrom),
      );
    }

    if (dateTo) {
      allActions = allActions.filter(
        (a) => new Date(a.createdAt) <= new Date(dateTo),
      );
    }
