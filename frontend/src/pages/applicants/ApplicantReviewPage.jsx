...
const ApplicantReviewPage = ({ darkMode, toggleDarkMode }) => {
  ...
  const handleAcceptApplicant = (applicantId) => {
      acceptMutation.mutate(applicantId._id);
  };
  ...
  const processApplicants = () => {
    ...
    let applicantsData = job.applicants.map(app => {
      return {
        ...
        status: 'pending' // Default status since it's not in your model
      };
    });
    
    // Filter by status (only if not 'all', otherwise show everything)
    if (filterStatus !== 'all') {
      // Since status isn't in your model, this will only show pending applicants
      // when filterStatus is 'pending', and no applicants otherwise
      applicantsData = applicantsData.filter(app => 
        filterStatus === 'pending' ? true : false
      );
    }
      ...
      <DashboardLayout darkMode={darkMode} toggleDarkMode={toggleDarkMode}>
        <div className="max-w-7xl mx-auto px-4 py-8">
          <EmptyState
            ...
            description={error.message || "There was an error loading the applicants."}
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
            ...
            <div className="flex flex-wrap justify-between items-start gap-4 mb-4">
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-gray-500 dark:text-gray-400">
                    ${job.budget} • Posted on {format(new Date(job.createdAt), 'MMM d, yyyy')}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center text-gray-600 dark:text-gray-400">
                  ...
                  <span>{filteredApplicants.length} applicant{filteredApplicants.length !== 1 ? 's' : ''}</span>
                </div>
        ...
        {filteredApplicants.length === 0 ? (
          <EmptyState
            ...
            actionText={filterStatus !== 'all' ? "Show All Applicants" : "Edit Job"}
            actionLink={filterStatus !== 'all' ? null : `/employer/jobs/${jobId}/edit`}
            onAction={filterStatus !== 'all' ? () => setFilterStatus('all') : null}
          />
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* Applicant list */}
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-grow min-w-0">
                            ...
                            <div className="flex items-center mt-1 text-sm text-gray-500 dark:text-gray-400">
                              {applicant.freelancer?.averageRating && (
                                <div className="flex items-center mr-3">
                                  <Star className="h-3.5 w-3.5 text-yellow-400 mr-1" fill="currentColor" />
                                  <span>{applicant.freelancer.averageRating.toFixed(1)}</span>
                                </div>
                              )}
                              <div className="flex items-center">
                ...
                >
                  <div className="p-6">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h2 className="text-xl font-bold text-gray-900 dark:text-white">
                            {selectedApplicant.freelancer?.name || 'Anonymous Freelancer'}
                          </h2>
                              ...
                              <div className="flex items-center mr-4">
                                <div className="flex">
                                  {[...Array(5)].map((_, i) => (
                                    <Star 
                                      key={i} 
                                      className={`h-4 w-4 ${i < Math.floor(selectedApplicant.freelancer.averageRating) ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-600'}`}
                                      fill={i < Math.floor(selectedApplicant.freelancer.averageRating) ? 'currentColor' : 'none'}
                                    />
                                  ))}
                                </div>
                    ...
                    <div className="mb-6">
                      ...
                      <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 mb-4 border border-gray-100 dark:border-gray-600">
                        ...
                        <p className="text-gray-600 dark:text-gray-400">
                          This freelancer applied to your job posting. You can view their profile details below to determine if they're a good fit for your project.
                        </p>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      {/* Skills */}
                      {selectedApplicant.freelancer?.skills && selectedApplicant.freelancer.skills.length > 0 && (
                        <div className="mb-4">
                      ...
                      {/* Experience */}
                      {selectedApplicant.freelancer?.experience && selectedApplicant.freelancer.experience.length > 0 && (
                        <div className="mb-4">
                      ...
                      {/* Ratings */}
                      {selectedApplicant.freelancer?.ratings && selectedApplicant.freelancer.ratings.length > 0 && (
                        <div>
                            ...
                            {selectedApplicant.freelancer.ratings.slice(0, 3).map((review, index) => (
                              <div key={index} className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3 border border-gray-100 dark:border-gray-600 hover:shadow-sm transition-all duration-200">
                                ...
                                <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">
                                  By {review.from?.name || 'Employer'} on {format(new Date(review.createdAt || Date.now()), 'MMM d, yyyy')}
                                </p>
                                <p className="text-gray-600 dark:text-gray-400">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden h-full flex items-center justify-center p-8 border border-gray-100 dark:border-gray-700">
                  <div className="text-center">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 max-w-md">
                      Click on an applicant from the list to view their profile information and make a hiring decision.
                    </p>
                  </div>
                </div>
              )}
