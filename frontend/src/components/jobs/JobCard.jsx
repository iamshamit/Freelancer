...
import { Briefcase, Calendar, Users, Clock, Sparkles, IndianRupee } from "lucide-react";
...
const JobCard = ({ job, isEmployer = false, showRecommendedBadge = false }) => {
  ...
  const domainObj =
    typeof domain === "object"
      ? domain
      : { name: domain || "General", color: "default" };
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start mb-4">
        <Badge
          variant={domainObj.color || "primary"}
          className="flex items-center gap-1"
        >
          {domainObj.icon && (
            <span className="material-icons text-sm">{domainObj.icon}</span>
          )}
          {domainObj.name}
        </Badge>
        {status && (
